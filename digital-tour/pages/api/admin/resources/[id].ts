...
export default async function handler(
  ...
  try {
    ...
    const autoUnlockIfExpired = async (resource: Resource) => {
      if (
        resource.locked_by &&
        resource.lock_expires_at &&
        new Date() > new Date(resource.lock_expires_at)
      ) {
    ...
    if (req.method === "POST") {
      if (!adminIdFromHeader)
        return res.status(401).json({ message: "x-admin-id required" });
      ...
      try {
        ...
        if (action === "lock") {
          if (resource.locked_by) {
            ...
            return res
              .status(409)
              .json({ message: `Locked by admin ${resource.locked_by}` });
          }
        ...
        if (action === "unlock") {
          if (resource.locked_by !== adminIdFromHeader) {
            ...
            return res
              .status(403)
              .json({ message: "Can only unlock your own lock" });
          }
          await resource.update(
            {
              locked_by: null,
        ...
        await t.rollback();
        console.error(
          `POST transaction failed for resource ${resourceId}:`,
          err
        );
        throw err;
      }
      ...
      try {
        ...
        if (status === "approved") {
          ...
          if (!rejection_reason || rejection_reason.trim().length < 10) {
            ...
            return res
              .status(400)
              .json({ message: "Rejection reason required (min 10 chars)" });
          }
        ...
        if (status === "approved") {
          ...
          const listingData = {
            ...
            price: updates.price ? Number(updates.price) : 0.0,
        ...
        await t.rollback();
        console.error(
          `PUT transaction failed for resource ${resourceId}:`,
          err
        );
        return res
          .status(500)
          .json({ message: "Failed to process resource and sync listing." });
      }
    }
