...
export function useNotifications(userId?: number, viewMode: ViewMode = "user") {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    setLoading(true);
    try {
      /** * FIX START: Convert "all" to undefined
       * This ensures TypeScript doesn't complain about "all" not matching
       * the service's expected parameters.
       */
      const serviceMode = viewMode === "all" ? undefined : viewMode;

      const data = await notificationService.getByUser(userId, serviceMode);
      // FIX END
